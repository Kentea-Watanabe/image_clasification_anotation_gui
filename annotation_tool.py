...
# image_width = 1000  # 表示画像の幅
# image_height = 850  # 表示画像の高さ
# ----------------------------------------------------------------------
class MainWindow:
    # ----------------
    ...
    def init_window(self):
        ...
        # 画像サイズを取得
        img = Image.open(
            os.path.join(self.images_dir, self.images_list[self.current_image_num])
        )
        ...
        for i, c in enumerate(self.classes):
            ...
            self.button_class.append(
                Button(
                    self.main,
                    text="{} ({})".format(c, key),
                    command=self.labeling(class_num=i),
                    width=10,
                )
            )
        ...
        self.set_message()
        # クラスを表示するラベル
        self.label_image_class = Label(
            self.main,
            textvariable=self.message_image_class,
            width=50,
            font=font_label_class,
            background="#CCDDDD",
        )
        self.label_image_class.grid(row=1, columnspan=7)
        # 現在の画像番号を表示するラベル
        self.label_image_index = Label(
            self.main, textvariable=self.message_image_index, font=font_label_index
        )
    ...
    def set_message(self):
        self.message_image_index.set(
            "{}/{}".format(str(self.current_image_num + 1), str(self.images_num))
        )
        self.message_image_class.set(
            "{}".format(self.get_class_name(self.images_list[self.current_image_num]))
        )
    ...
    def set_image(self, e=None):
        ...
        image_height, image_width = img.size
        img = img.resize(
            (int(image_width * 0.5), int(image_height * 0.5)), Image.LANCZOS
        )
